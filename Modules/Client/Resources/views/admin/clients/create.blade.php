...
@section('content-header')
    <h1>
        {{ trans('client::clients.title.create client') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.client.client.index') }}">{{ trans('client::clients.title.clients') }}</a></li>
        <li class="active">{{ trans('client::clients.title.create client') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.client.client.store'], 'method' => 'post']) !!}
    <div class="row">
        <div class="col-md-10">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('client::admin.clients.partials.create-fields', ['lang' => $locale])
                        </div>
                    @endforeach
