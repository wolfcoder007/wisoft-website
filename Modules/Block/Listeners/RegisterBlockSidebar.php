...
namespace Modules\Block\Listeners;
...
class RegisterBlockSidebar implements \Maatwebsite\Sidebar\SidebarExtender
{
    /**
     * @var Authentication
     */
    ...
    public function extendWith(Menu $menu)
    {
        $menu->group(trans('core::sidebar.content'), function (Group $group) {
            $group->item(trans('block::blocks.title.blocks'), function (Item $item) {
                ...
                $item->item(trans('block::blocks.title.blocks'), function (Item $item) {
                    $item->icon('fa fa-diamond');
                    $item->weight(0);
